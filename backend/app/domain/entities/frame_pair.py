...
class FramePair(BaseEntity):
    ...
    def _validate(self) -> None:
        ...
        if self.frame_a.id == self.frame_b.id:
            raise DomainValidationError(
                "FramePair cannot contain the same frame twice."
            )

        if self.frame_a.resolution != self.frame_b.resolution:
            raise DomainValidationError("Frame dimensions must match.")

        if self.frame_a.metadata.channels != self.frame_b.metadata.channels:
    ...
    def time_gap(self) -> timedelta:
        return self.frame_b.timestamp.value - self.frame_a.timestamp.value
    ...
    def __str__(self) -> str:
        return (
            f"FramePair(" f"{self.frame_a.path.name} -> " f"{self.frame_b.path.name})"
        )
